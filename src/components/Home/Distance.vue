<template>
  <section class="mb-5" >
    <h3 class="mb-5">Convertissez vos distances en millimétre, centimètre, décimètre, mètre, décamètre, hectomètre et kilomètre </h3>
    <Layout>
      <div class="container">
        <div class="row">
          <div class="col-sm pt-5 p-3 p-sm-3">
            <input class="form-control form" v-model="data.value"  name="value" v-validate="'required|numeric'" placeholder="Ex: 101" type="text">
            <div v-if="isError" class=" mt-3 alert alert-danger" role="alert">
              {{errors.first('value')}}
            </div>
          </div>
          <div class="col-sm p-3 p-sm-3">
            <model-select :options="distance"
                          v-model="data.from_distance"
                          class="form pt-3">
            </model-select>
          </div>
          <div class="col-sm p-3 p-sm-3">
  ...
  methods:{
    convert(){
      this.$validator.validateAll().then(result => {
        if (result) {
          this.result = evaluate(`${this.data.value} ${this.data.from_distance} to ${this.data.to_distance}`)
        }
        else
        {
          this.isError = true;
